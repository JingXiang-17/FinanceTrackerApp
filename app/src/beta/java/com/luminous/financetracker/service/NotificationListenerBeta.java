...
public class NotificationListenerBeta extends NotificationListenerService {
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        if (isLoggingEnabled && AMOUNT_PATTERN.matcher(lowerText).find()) {
            String cleanText = text.replaceAll("[\\r\\n]+", " ").replace("\"", "\"\"");
            String category = determineCategory(lowerText);
            writeToCsv(cleanText, category);
        }

        // --- EARLY EXITS (Including the new Pre-Auth fix) ---
        if (PRE_AUTH_PATTERN.matcher(lowerText).find()) return; // STOPS PRE-AUTHORIZATION DOUBLE LOGS
        ...
        if (finalAmount > 0) {
            // Extract Merchant Name dynamically
    ...
    private String determineCategory(String lowerText) {
        if (PRE_AUTH_PATTERN.matcher(lowerText).find()) return "JUNK";
    ...
    private String determinePaymentMethod(String packageName) {
        return switch (packageName) {
            case Constants.PKG_TNG -> Constants.PAY_TNG;
            case Constants.PKG_SHOPEE -> Constants.PAY_SHOPEE;
            case Constants.PKG_LAZADA -> Constants.PAY_LAZADA;
            case Constants.PKG_GRAB -> Constants.PAY_GRAB;
            case Constants.PKG_TEMU -> Constants.PAY_TEMU;
            case Constants.PKG_WECHAT -> Constants.PAY_WECHAT;
            case Constants.PKG_ALIPAY -> Constants.PAY_ALIPAY;
            case Constants.PKG_BOOST -> Constants.PAY_BOOST;
            case Constants.PKG_HLB -> Constants.PAY_HLB;
            case Constants.PKG_RHB -> Constants.PAY_RHB;
            case Constants.PKG_BANK_RAKYAT -> Constants.PAY_BANK_RAKYAT;
            case Constants.PKG_MAE -> Constants.PAY_MAE;
            case Constants.PKG_RYT -> Constants.PAY_RYT;
            case Constants.PKG_CIMB -> Constants.PAY_CIMB;
            case Constants.PKG_OCBC -> Constants.PAY_OCBC;
            default -> Constants.PAY_UNKNOWN;
        };
    }
}